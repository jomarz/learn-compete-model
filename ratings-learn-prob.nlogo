    ...
    if random 100 < 100 [
      sprout 1 [
        ifelse random 100 >= pctFemale [ ; Create male player
          set sex "M"
          set rating precision (random-normal 1941 247) 0
        ]
        [ ; Else, create female player
          set sex "F"
          set rating precision (random-normal 1813 264) 0
        ]
  ...
  ask turtles [
    if not playing? [
      ...
      ask turtle-set one-of turtles with ([group = [group] of myself and who != [who] of myself and not playing?] ) [ ; ask another player in the same tournament. This will be player 2
        ...
        let bonus1 getBonus rating1 rating2
        let bonus2 getBonus rating2 rating1
    ...
    if renewal [ ; If player retirement is on, players are renewed after maxAge
      ...
      if age > maxAge [ ; Old player retires and a new player enters the scene
        set age  1
        if sex = "M" [
          set rating random-normal (mean ([rating] of turtles with [sex = "M"]) - 279) 279
        ]
        if sex = "F"[
          set rating random-normal (mean ([rating] of turtles with [sex = "F"]) - 261) 261
        ]
        set color getColor rating sex
      ]
    ]
...
to-report getBonus [rating1 rating2]
  ...
  report maxBonus * exp( - ((rating2 - (rating1 + idealDiff)) ^ 2 / (2 * (bonusWidth ^ 2))))
end
...
"" ""
PENS
"M" 1.0 2 -14439633 true "" "plot mean [rating] of max-n-of 20 turtles with [sex = \"M\"] [rating]"
"F" 1.0 2 -3844592 true "" "plot mean [rating] of max-n-of 20 turtles with [sex = \"F\"] [rating]"
...
"" ""
PENS
"Top 20" 1.0 2 -16777216 true "" "plot (mean [rating] of max-n-of 20 turtles with [sex = \"M\"] [rating] - mean [rating] of max-n-of 20 turtles with [sex = \"F\"] [rating]) * 100 / mean [rating] of max-n-of 20 turtles with [sex = \"M\"] [rating]"
"All players" 1.0 0 -6459832 true "" "plot (mean [rating] of turtles with [sex = \"M\"] - mean [rating] of turtles with [sex = \"F\"] ) * 100 / mean [rating] of turtles with [sex = \"M\"]"
"pen-2" 1.0 2 -14454117 false "" "plot 0"
